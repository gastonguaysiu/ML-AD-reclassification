...
long_data <- data %>%
  ...
  mutate(Group = factor(Group, levels = c("cl1_age", "cl1_adj_DNAm_age", "cl2_age", "cl2_adj_DNAm_age", "cl3_age", "cl3_adj_DNAm_age")))
...
custom_colors <- c("cl1_age" = "green", "cl1_adj_DNAm_age" = "lightgreen",
                   "cl2_age" = "yellow", "cl2_adj_DNAm_age" = "lightyellow",
                   "cl3_age" = "darkred", "cl3_adj_DNAm_age" = "red")
...
summary_cl1 <- summarize_subset(data, "cl1")
summary_cl2 <- summarize_subset(data, "cl2")
summary_cl3 <- summarize_subset(data, "cl3")

# Combine the summaries into a single data frame
group_summary <- bind_rows(summary_cl1, summary_cl2, summary_cl3)
...
# write.csv(tukey_df, "anova_HSD_ML.csv", row.names = TRUE)
